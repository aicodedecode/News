...
def format_message(current_affairs, history_polity):
    ...
    message += "<i>Updated daily at 8:00 AM</i>"
...
def daily_scrape_and_post():
    ...
    try:
        ...
        if not current_affairs:
            logger.warning("⚠️ No current affairs found")
        if not history_polity:
            logger.warning("⚠️ No history/polity content found")
        ...
        if current_affairs or history_polity:
            ...
            logger.warning("⚠️ No content to post")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🤖 Telegram Daily News Scraper")
    print("=" * 60)
